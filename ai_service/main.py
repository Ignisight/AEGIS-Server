...
async def verify_face(req: VerifyRequest):
    try:
        ...
        if is_spoof:
            should_flag = True
            flag_reason = f"SPOOF_ALERT: Moire={moire:.1f}, Var={var:.1f}"
            verified = False 
            
        if not req.liveness_verified:
            should_flag = True
            flag_reason = "SEC_BYPASS: App-side liveness signal missing."
            verified = False 

        if drift < 0.78: # Slightly stricter for ArcFace
            should_flag = True
        ...
        return {
            ...
            "anti_spoofing": {"variance": var, "moire": moire}
        }
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
