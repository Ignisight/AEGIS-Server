...
async function sendEmail(to, subject, html) {
  ...
  // 1. Prefer Brevo (HTTP API is more reliable in production)
  if (BREVO_API_KEY) {
    try {
      ...
      // Fallback to Gmail if Brevo fails and Gmail is configured
    }
  }
...
// Student notifications (now just an alias to the unified sendEmail)
...
function verifyAppSecret(req, res, next) {
  ...
  if (!isValidSignature) {
     ...
     logger.warn(debugMsg, { 
        ...
        logNonce: nonce,
        logPayloadLen: payloadData.length,
        logPayloadPrefix: payloadData.slice(0, 50) 
     });
